...
**适用**：组合或参数优化在能定义可行编码、目标 f 或等价的适应度且用单点邻域不划算时；大规模时常与局部搜索、修复混合。

**核心**：在一代代循环中，用选择调节探索与利用的取舍，用交叉在编码空间重组信息，用变异保多样性，用替换决定谁进入下一代、是否精英留档。下框为主循环的完整流程；其后的四节对选择、交叉、变异、替换分别作展开与常见算子说明。

---

## 算法（最小化示意）

以最小化 f(s) 为例；s 为个体编码（二进串、排列、实向量等）。适应度与 f 的关系（直接用 f、取倒数/加常数防负等）须自始至终结一致，便于不同代之间比较与选择。
...
### 2. 一代迭代（重复直至终止条件）
- **选择**：由当前 P 中按选择规则选出一批用于繁殖的父本（可重复选同一好个体，依锦标赛、轮盘、排序等算子；细节见下节「选择」）。  
- **交叉**：对父本两两或按算子配对，以交叉率 p_c 在编码上执行交叉，得子代中间集；未发生交叉时可将父本复制为子代。  
- **变异**：对子代个体以变异率 p_m 执行变异（如比特翻、对换、高斯微扰等），保持或修复可行性。  
- **评价与替换**：对子代及（若需）与父代联合评价 f；按完全代际替换、稳态只换部分个体、精英等策略组成下一代 P；若出现更优个体则更新 s⋆（细节见下节「替换」）。
...
框内为可直接依序实现的主循环；各步可用的具体算子、参数与变体，见下四节分述。
...
**(μ+λ) / (μ,λ) 式叙述**：有文献用「μ 个父、λ 个子」：(μ+λ) 为父母与子联合取最好的 μ 个；(μ,λ) 为只从子里选 μ 个，不保留未选父本（更激进）。经典 GA 叙述常不严格用这套符号，但思想与精英、稳态可对照。
...
- 相对单点方法（[模拟退火](simulated-annealing.md)、[禁忌搜索](tabu-search.md)）：多解并行搜索，由选择—交叉在编码上大跳，不必每步定义邻域；但算子与调参与问题强绑定。  
- 常与局部搜索（memetic）或多目标族结合，本页不展开。  

## 参考阅读

- Holland 的经典 GA 思想；Michalewicz 等关于实参/排列编码与算子的整理；各应用领域的「编码 + 选择 + 交叉 + 替换」可复现案例。  
- 实现时建议把选择、交叉、变异、替换各自做成可替换的模块，日志记录代数、最优值与算子设置，方便对比基线。
