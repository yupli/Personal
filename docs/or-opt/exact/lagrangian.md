...
当整数规划或混合整数规划可写成「一组相对 **简单且结构化** 的约束与非退化可行域 $X$」加上一组「书写冗长或使模型难解 **耦合约束**」$D\mathbf{x}\le\mathbf{d}$ 时，常可把后者松弛进目标并转而重复求解 $\max_{\mathbf{x}\in X}(\mathbf{c}^\top\mathbf{x}+\mathbf{u}^\top(\mathbf{d}-D\mathbf{x}))$，再配合乘子优化或其它迭代给出 **原问题上界或下界**（极大或极小情形而定）、或为分支定界供可行松弛。**典型**：设施选址 / 选址分配中与耦合有关的切割容量约束、[旅行商](../classic-models/traveling-salesman-problem.md) 中的海量子回路不等式（在理论上可先松弛后对 $\mathbf{u}$ 迭代）、车辆路由中与装载耦合的大规模耦合不等式写不进紧凑 MILP 时的定性近似边界。**不适用**：难约束不可剥离且无法在去掉它们之后仍能高效求解子问题的情形；或子问题本身仍为 NP-hard 且没有比原来更简单时才慎用单独 LR。
