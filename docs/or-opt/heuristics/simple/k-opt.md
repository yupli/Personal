...
**注**：在图中读「反转」时，可对照 Mermaid 表示的一段序从 v<sub>i+1</sub>…v<sub>j</sub> 变成 v<sub>j</sub>…v<sub>i+1</sub>。

```mermaid
%% 2-opt 邻域 = 把 vi+1 到 vj 段反序
flowchart LR
  s1[vi 与 vj+1 外侧] --> mid1[原序 vi+1 到 vj]
  s2[vi 与 vj+1 外侧] --> mid2[反序 vj 到 vi+1]
```
