...
上节内嵌的 SVG 为页面内可读的静态图；你若有可合法使用的参考图，也可放入 `docs/or-opt/resources/` 并在本页以 `![说明](../../resources/xxx.png)` 引用。有向、多车场、时间窗时，节约式与端点/可接规则要按题设写清。算法不保证 CVRP 全局最优，但实现快、常与 [k-opt 系列](k-opt.md) 或元启发式搭配，或作初始上界。
