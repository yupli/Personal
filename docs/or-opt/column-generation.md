...
显然，当 RMP 的解中所有 $y_r$ 取值均为整数时，该解才是原问题的可行解。根据前文的描述，我们用**分支定价算法**来求解该问题。

#### 算法主要步骤

分支定价算法的主要思路如下：

1. **初始化**：为 RMP 生成一些初始的列，并构建初始 RMP（公式15）

2. **求解松弛问题**：求解 RMP 的线性松弛（公式19-21），得到对偶变量 $\pi$

3. **列生成**：根据对偶变量更新子问题目标函数，求解子问题（ESPPRC，公式23-30），得到一些检验数为负的路径，将其加入 RMP 中

4. **循环迭代**：循环步骤（2）和（3），直到没有新的检验数为负的路径生成为止
...
| 步骤 | 操作 | 条件/结果 |
|------|------|-----------|
| 初始化 | 构建RMP，加入初始路径集合 $R'$ | - |
| 求解松弛 | 得到对偶变量 $\pi$ | - |
| 求解子问题 | ESPPRC，寻找检验数为负的列 | 找到则添加列；未找到则检查整数性 |
