...
元启发式是在编码解表示上，用通用高层迭代机制在探索与利用间平衡的启发式类方法。分类与三类边界见 [启发式算法总览](heuristic-algorithms.md)。与总览中「后主体循环」的体例一致，本目录下各篇在展开记号、机制等正文后、在给出主循环框之前，会写清调参/实现要点（个别篇含内层子过程小框，见各文相应小节）。
