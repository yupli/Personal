...
元启发式是在编码解表示上，用通用高层迭代机制在探索与利用间平衡的启发式类方法。分类与三类边界见 [启发式算法总览](heuristic-algorithms.md)。与总览中「先概念与要点、后主体循环」的体例一致，本目录下各篇在给出主循环框之前，会先写清概念与调参/实现要点（个别篇含内层子过程小框，见各文前言）。
