...
**写作体例**：凡带主循环的元启发式子页，开篇为一段总述并链到本总览；正文先展开记号、机制、算子或参数含义等，最后以 `algorithm` 容器给出主体循环，便于对照框图实现。若另含子过程（如 VNS 内层的 VND），子过程仍可在前文中用独立小框收束，对外的整体主循环仍用 `algorithm` 收束并排在上述内容之后。
