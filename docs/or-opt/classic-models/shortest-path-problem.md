...
**注**：上式中 $\text{out}(i)$ 表示自节点 $i$ 出发的边 $e$ 的全体，$\text{in}(i)$ 表示进入 $i$ 的边 $e$ 的全体（与常见网络流记法中「出弧 / 入弧」的约定一致；无向边可在建模时拆成两条有向弧或采用别种写法，此处不展开）。
...
），在适当条件下仍存在整数最优解，因而整数规划与盒约束下线性规划在最优解意义上可讨论等价性。理论背景见 [最优整数解特性与幺模矩阵](integer-solutions-unimodular-matrices.md)。
...
**注**：上列 (1)–(7) 是「最短路 as 0–1 路径流 / LP 松弛」的常见写法。实现时需注意有向边方向与 $\text{in}/\text{out}$ 的唯一定义，以保证可复现性。
