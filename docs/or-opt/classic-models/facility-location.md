...
**无容量限制的设施选址问题**（Uncapacitated Facility Location Problem, UFLP；亦常记为简单工厂选址/简单设施选址的整数规划形式）是 FLP 的基础版本：在候选点集合中决定开哪些设施，使全体客户需求被满足，并最小化**运输（或服务）成本与固定开设成本之和**。若将运输成本在模型中略去，则与覆盖、集合覆盖等结构会更接近，文献中常对比讨论。

设**候选设施**集合为 $N = \{1,2,\ldots,n\}$，**客户**集合为 $M = \{1,2,\ldots,m\}$。在设施 $j$ 开设的**固定费**为 $f_j$，由设施 $j$ 服务客户 $i$ 的**单位意义费用**为 $c_{ij}$（教材中 $x_{ij}$ 多解释为需求被 $j$ 满足的比例，且各客户总需求在归一化后由 $\sum_{j} x_{ij}=1$ 表出）。**决策变量**为：
