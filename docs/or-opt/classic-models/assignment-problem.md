...
指派问题在图论与组合优化中常视为一种特殊的二部图匹配问题：一边节点表示司机、另一边表示订单（或更一般的「人」与「任务」），$c_{ij}$ 在边上赋权。示意时常画二部图：$D_i$（$i=1,2,3,4$）记一侧顶点、$R_i$ 记另一侧，虚线表示可连边（可能匹配），实线表示选中的一组匹配。具体工程应用与案例的展开可参见文献 Xu et al. (2018)。
